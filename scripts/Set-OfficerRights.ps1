...
# closed for PATH-selected programs, one line lower down. InstallVerifyLib is
# itself the code that authenticates this privileged script tree, so executing
# it from disk before authenticating it would make the later verdict
# self-referential. Pin its canonical UTF-8/LF bytes here, hash the in-memory
# text, and execute that exact verified text (never reopen the path).
